    ...
    public function ajax_list() {
        ...
        foreach ($list as $item) {
            ...
            foreach ($tag_siswa as $detail) {
                if($detail->STATUS_SETUP) $lunas = TRUE;
            ...
            $row[] = '<button type="button" class="btn btn-sm btn-'.(count($tag_siswa) > 0 ? 'danger' : 'info').'" onclick="proses_tagihan(this)" data-siswa="'.$item->ID_SISWA.'" data-hapus="'.(count($tag_siswa) > 0 ? '1' : '0').'" data-nama="'.$item->NIS_SISWA.' - '.$item->NAMA_SISWA.'" data-dept="'.$item->DEPT_TINGK.'" data-tagihan="'. urlencode(json_encode($tag_id)).'" '.($lunas ? 'disabled' : '').' title="'.($lunas ? 'Tagihan ada yang telah dibayar. Kembalikan terlebih dahulu sebelum menghapus tagihan ini.' : '').'"><i class="fa fa-'.(count($tag_siswa) > 0 ? 'remove' : 'plus').'"></i></button>';
