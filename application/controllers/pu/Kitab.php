...
class Kitab extends CI_Controller {
    ...
    public function index($ATURAN = TRUE) {
        $jadwal = $this->kitab->get_rows('testing_jadwal', array('TA_TW' => $this->session->userdata('ID_TA_ACTIVE')));
    ...
    public function ajax_list() {
        ...
        $list = $this->kitab->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->kitab->count_all(),
            "recordsFiltered" => $this->kitab->count_filtered(),
            "data" => $data,
        );
    ...
    private function get_data_jadwal() {
        $data = array(
            'WAKTU' => array(
                'L' => $this->kitab->get_rows('testing_waktu', array('JK_TW' => 'L','TA_TW' => $this->session->userdata('ID_TA_ACTIVE'))),
                'P' => $this->kitab->get_rows('testing_waktu', array('JK_TW' => 'P','TA_TW' => $this->session->userdata('ID_TA_ACTIVE')))
            ),
            'RUANG' => array(
                'L' => $this->kitab->get_rows('testing_ruang', array('JK_TR' => 'L','TA_TR' => $this->session->userdata('ID_TA_ACTIVE'))),
                'P' => $this->kitab->get_rows('testing_ruang', array('JK_TR' => 'P','TA_TR' => $this->session->userdata('ID_TA_ACTIVE')))
            ),
            'MAPEL' => $this->kitab->get_rows('testing_mapel', array('TA_TM' => $this->session->userdata('ID_TA_ACTIVE'))),
            'JUMLAH' => array(
        ...
        foreach ($data['RUANG'] as $JK_RUANG => $DATA_RUANG) {
            foreach ($DATA_RUANG as $DETAIL_RUANG) {
                foreach ($data['WAKTU'][$JK_RUANG] as $DETAIL_WAKTU) {
                    ...
                    $data['MAPEL_JADWAL'][$DETAIL_RUANG->ID_TR][$DETAIL_WAKTU->ID_TW] = $this->kitab->get_row('testing_jadwal', $data_jadwal);
                    for ($kl = 0; $kl < count($this->tugas); $kl++) {
                        $data_jadwal['TUGAS_TP'] = $this->tugas[$kl];
                        
                        $data['JADWAL'][$DETAIL_RUANG->ID_TR][$DETAIL_WAKTU->ID_TW][$data_jadwal['TUGAS_TP']] = $this->kitab->get_row('testing_jadwal', $data_jadwal);
                    }
                }
            }
        }
    ...
    public function lihat_jadwal($cetak = FALSE) {
        ...
        if($cetak) 
            $this->generate->backend_view('pu/kitab/cetak', $data);
        else
            $this->generate->backend_view('pu/kitab/view', $data);
    }
    ...
    public function form_aturan($ATURAN) {
        ...
        if(count($data['WAKTU']['L']) == 0 || count($data['WAKTU']['P']) == 0 || count($data['RUANG']['L']) == 0 || count($data['RUANG']['P']) == 0 || count($data['MAPEL']) == 0) {
            $this->kitab->reset_jadwal_ta();
        }
        
        if($ATURAN)
            $this->generate->backend_view('pu/kitab/form', $data);
        else 
            $this->generate->backend_view('pu/kitab/form_jadwal', $data);
    } 
...
//        }
    ...
    public function tambah_aturan() {
        ...
        for ($i = 0; $i < $data['WAKTU_LK']; $i++) {
            $this->kitab->save('testing_waktu',$data_waktu);
        }
        $data_waktu['JK_TW'] = 'P';
        ...
        for ($i = 0; $i < $data['RUANG_LK']; $i++) {
            $this->kitab->save('testing_ruang',$data_ruang);
        }
        $data_ruang['JK_TR'] = 'P';
        ...
        for ($i = 0; $i < $data['MAPEL']; $i++) {
            $this->kitab->save('testing_mapel',$data_mapel);
        }
        
        $this->generate->output_JSON(array('status' => 1, 'url' => site_url('pu/kitab')));
    }
    ...
    public function simpan_aturan() {
        ...
        $data_ruang_lk = $this->kitab->get_rows('testing_ruang', array('JK_TR' => 'L','TA_TR' => $this->session->userdata('ID_TA_ACTIVE')));
        ...
        for ($i = 0; $i < count($data['ID_TW']); $i++) {
            ...
            $this->kitab->update('testing_waktu', $where_waktu, $data_waktu);
        }
        ...
        for ($i = 0; $i < count($data['ID_TM']); $i++) {
            ...
            $this->kitab->update('testing_mapel', $where_mapel, $data_mapel);
        }
        ...
        for ($i = 0; $i < count($data['ID_TR']); $i++) {
            ...
            $this->kitab->update('testing_ruang', $where_ruang, $data_ruang);
        }
        
        $this->generate->output_JSON(array('status' => 1, 'url' => site_url('pu/kitab/index/0')));
    }
    ...
    public function simpan_denah() {
        ...
        $this->kitab->delete_by_where('testing_jadwal', array('TA_TP' => $this->session->userdata('ID_TA_ACTIVE')));
        ...
        for ($i = 0; $i < count($data['RUANG_TP']); $i++) {
            for ($k = $j; $k < ($j + 5); $k++) {
                ...
                $this->kitab->save('testing_jadwal', $data_jadwal);
            }
    ...
    public function cetak() {
        ...
        $this->load->view('backend/pu/kitab/cetak', $result);
    }

    public function input_nilai() {
        $this->generate->backend_view('pu/kitab/nilai');
    }
