        ...
        foreach ($kelas as $detail_kelas) {
            foreach ($hari as $detail_hari) {
                ...
                foreach ($data['jam_pelajaran'][$detail_kelas->ID_KELAS][$detail_hari->ID_HARI] as $detail_jp) {
                    $result = $this->jadwal->get_jadwal_kelas($detail_kelas->ID_KELAS, $detail_hari->ID_HARI, $detail_jp->ID_MJP);
                    if (count($result) > 0)
                        $data['jadwal'][$detail_kelas->ID_KELAS][$detail_hari->ID_HARI][$detail_jp->ID_MJP] = $result;
                }
            }
