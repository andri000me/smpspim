...
//	xml_print($this->dom);
    ...
    private function write_jam_pelajaran_data($jenjang, $jk) {
        $where = array();
        if($jenjang != 'ALL') $where['DEPT_MJP'] = $jenjang;
        else $where['DEPT_MJP'] = 'AL';
        $where['JK_MJP'] = $jk;
    ...
    private function write_kelas_data($jenjang, $jk) {
        ...
        if($jenjang != 'ALL') $where['DEPT_TINGK'] = $jenjang;
    ...
    private function write_mapel_data($jenjang) {
        ...
        if($jenjang != 'ALL') $where['DEPT_MAPEL'] = $jenjang;
...
//        echo json_encode($jam_pelajaran_db);
        ...
        foreach ($xml->cards as $card) {
            foreach ($card as $element) {
                ...
                if ($hari == NULL) $hari = $this->get_daydef($xml->daysdefs, (string)$element->attributes()['days']);
...
//        exit();
    ...
    private function get_id($parent, $id) {
        foreach ($parent as $child) {
            foreach ($child as $element) {
                ...
                if (((string)$attr->id) == $id) return ((string)$attr->short);
            }
        }
    }
    ...
    private function get_day($parent, $id) {
        foreach ($parent as $child) {
            foreach ($child as $element) {
                $attr = $element->attributes();
                if (((string)$attr->day) == $id) return ((string)$attr->short);
            }
        }
    }
    ...
    private function get_daydef($parent, $id) {
        foreach ($parent as $child) {
            foreach ($child as $element) {
                $attr = $element->attributes();
                if (((string)$attr->days) == $id) return ((string)$attr->short);
            }
        }
    }
}
