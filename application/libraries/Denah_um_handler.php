...
class Denah_um_handler{
    ...
    private function generate_denah($mode, $jk) {
        ...
        if($mode == 'UM') $peserta = $this->CI->peserta_um->get_all_denah($jk);
        elseif($mode == 'US') $peserta = $this->CI->peserta_us->get_all_denah($jk);
...
//        exit();
        ...
        foreach ($temp_data['DATA'] as $index => $item) {
            ...
            if ($i == $jumlah_ruangan_tidak_sisa) {
                ...
                foreach ($temp_data['DATA'] as $key => $value) {
                    if ($key > $index) $jumlah_sisa_siswa += $value;
                }
                if ($jumlah_sisa_siswa < $jumlah_ruangan_tidak_sisa)  break;
            } else {
                $temp_i = $i + 1;
            }
        }
    ...
    public function cek_jumlah_input($data) {
        ...
        foreach ($temp_jumlah as $key => $value) {
            if ($data['JUMLAH'][$key] != $value) $status = FALSE;
        }
    ...
    private function urut_jenjang_terbesar($jenjang, $data) {
        ...
        foreach ($jenjang as $key => $value) {
            if ($temp_jenjang_single != $value) $temp_jenjang[] = $value;
            $temp_data[$value][] = $key;
    ...
    private function dalam_satu_jenjang($data_jenjang, $index_aturan, $index_cek) {
        ...
        foreach ($data_jenjang as $key => $value) {
            ...
            if ($key == $index_aturan) $finish = TRUE;
    ...
    public function proses_buat_denah($mode, $req_sisa = FALSE) {
        ...
        $result = array(
            'L' => $this->buat_denah($mode, $data_lk),
            'P' => $this->buat_denah($mode, $data_pr)
        );
    ...
    private function susun_data_sisa($sisa, $data) {
        ...
        foreach ($kumpulan_sisa as $tingkat_sisa => $jumlah_peserta) {
            if($temp_jenjang != $jenjang[$tingkat_sisa]) $ruang_sekarang = $ruang_terakhir;
            
            for ($i = 0; $i < $jumlah_peserta; $i++) {
                $jumlah_peserta_pertingkat = 0;
                foreach ($result[$ruang_sekarang] as $tingkat_cek => $jumlah_cek) {
                    if($jenjang[$tingkat_cek] == $jenjang[$tingkat_sisa]) $jumlah_peserta_pertingkat += $jumlah_cek;
                }
                
                if ($jumlah_peserta_pertingkat >= $maksimal_perruang_pertingkat[$ruang_sekarang]) $ruang_sekarang++;
        ...
        foreach ($result as $key => $aturan_denah) {
            ...
            if ($jumlah > 0) $sisa['ATURAN_DENAH'][$key] = $aturan_denah;
        }
    ...
    // MEMBUAT DENAH
    private function buat_denah($mode, $data) {
        $result = array();
        $jenjang = $data['JENJANG'];
        ...
        foreach ($data['ATURAN_DENAH_FINAL'] as $key => $value) {
            ...
            $jumlah_peruang = $jumlah_peserta > $data['RUANG'][0]['KAPASITAS_RUANG'] ? $jumlah_peserta : $data['RUANG'][0]['KAPASITAS_RUANG'];
...
//                                                (isset($result_denah[$key][$cek_bangku_kosong - $jumlah_perbaris - 1]) && $this->dalam_satu_jenjang($jenjang, $index_aturan, $result_denah[$key][$cek_bangku_kosong  - $jumlah_perbaris - 1]))
    ...
    public function atur_ulang_denah($mode) {
        ...
        $denah = json_decode($this->CI->aturan_denah->get_denah_psb(), TRUE);
