...
class Laporan_hafalan_model extends CI_Model {
    ...
    private function _get_table() {
        ...
        $this->db->join('md_pondok_siswa', 'ID_MPS=PONDOK_SISWA', 'LEFT');
    }

    public function get_data($group, $ta, $tingkat, $jenjang, $jk, $params = null) {
        if ($group == 'Jumlah Siswa') {
            $this->db->select('COUNT(ID_AS) AS data, NAMA_KELAS AS x_label, DEPT_TINGK, NAMA_PONDOK_MPS');
        } elseif ($group == 'Jumlah Siswa Setoran') {
            $this->db->select('COUNT(ID_PNH) AS data, NAMA_KELAS AS x_label, DEPT_TINGK, NAMA_PONDOK_MPS');
            $this->db->where('ID_PNH IS NOT NULL');
            $this->db->where('STATUS_PNH <> ', 'KELUAR');
        } elseif ($group == 'Jumlah Siswa Belum Setoran') {
            $this->db->select('(COUNT(ID_AS) - COUNT(ID_PNH)) AS data, NAMA_KELAS AS x_label, DEPT_TINGK, NAMA_PONDOK_MPS');
            ...
            $this->db->select('NAMA_KELAS AS x_label, DEPT_TINGK, NAMA_PONDOK_MPS');
        }

        if ($params != null) {
            if (isset($params['mode'])) {
                if ($params['mode'] == 'hafalan_perkelas') {
                    $this->db->where('NAMA_KELAS IS NOT NULL');
                    $this->db->group_by('ID_KELAS');
                    $this->db->order_by('x_label', 'ASC');
                } elseif ($params['mode'] == 'hafalan_perjenjang') {
                    $this->db->where('NAMA_KELAS IS NOT NULL');
                    $this->db->group_by('DEPT_TINGK');
                    $this->db->order_by('x_label', 'ASC');
                } elseif ($params['mode'] == 'hafalan_perpondok') {
                    $this->db->where('ID_MPS IS NOT NULL');
                    $this->db->where('NAMA_KELAS IS NOT NULL');
                    $this->db->group_by('ID_MPS');
                    $this->db->order_by('x_label', 'ASC');
                }
            }
        }
