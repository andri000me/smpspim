...
class Pencarian_model extends CI_Model {
    ...
    private function set_param($kata_kunci, $filter) {
        ...
        foreach ($filter as $detail) {
            if ($i == 0)
                $this->db->like($detail, $kata_kunci);
            else
                $this->db->or_like($detail, $kata_kunci);
    ...
    public function get_id_siswa($NIS_SISWA) {
...
//        $this->db->select('COUNT(ID_AKH) AS JUMLAH');
    ...
    public function get_poin_header($ID_SISWA, $ID_TA) {
        $this->db->select('*, SUM(POIN_KSH) AS JUMLAH_POIN_KSH, SUM(LARI_KSH) AS JUMLAH_LARI_KSH');
        $this->db->from('komdis_siswa_header');
        $this->db->where(array(
            'TA_KSH' => $ID_TA,
            'SISWA_KSH' => $ID_SISWA
    ...
    public function get_poin_detail($ID_SISWA, $ID_TA) {
        $this->db->from('komdis_siswa');
        $this->db->join('komdis_jenis_pelanggaran kjp', 'PELANGGARAN_KS=ID_KJP');
//        $this->db->join('akad_kehadiran akh', 'KEHADIRAN_KS=ID_AKH', 'LEFT');
        $this->db->where(array(
            'TA_KS' => $ID_TA,
            'SISWA_KS' => $ID_SISWA
        ));
    ...
    public function get_tindakan($ID_SISWA, $ID_TA) {
        $this->db->from('komdis_siswa_header');
        $this->db->join('komdis_tindakan', 'PELANGGARAN_HEADER_KT=ID_KSH');
        $this->db->join('komdis_jenis_tindakan kjt', 'TINDAKAN_KT=ID_KJT');
        $this->db->where(array(
            'TA_KSH' => $ID_TA,
            'SISWA_KSH' => $ID_SISWA
        ));
