...
class Absen_siswa_model extends CI_Model {
    ...
    private function _get_table($ID_KELAS, $JENIS_AKH = NULL, $TANGGAL_AKH = NULL) {
        ...
        if (($JENIS_AKH != NULL) && ($TANGGAL_AKH != NULL))
            $this->db->join('akad_kehadiran akh', $this->table . '.SISWA_AS=akh.SISWA_AKH AND akh.TA_AKH=' . $this->session->userdata('ID_TA_ACTIVE') . ' AND (akh.JENIS_AKH=' . $JENIS_AKH . ' OR akh.JENIS_AKH=1) AND akh.TANGGAL_AKH="' . $this->date_format->to_store_db($TANGGAL_AKH) . '"', 'LEFT'); //  AND akh.CAWU_AKH=' . $this->session->userdata('ID_CAWU_ACTIVE') . ' AND
        $this->db->where(array(
