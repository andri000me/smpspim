...
class Peserta_testing_tt_model extends CI_Model {

    var $table = 'akad_siswa';
    var $column = array('NAMA_TA','NIS_SISWA', 'NAMA_SISWA','JK_SISWA','DEPT_TINGK','NAMA_TINGK','NAMA_KELAS','NAMA_PEG', 'LULUS_AS','NILAI_TN', 'ID_AS');
    ...
    private function _get_table($quran = true) {
        ...
        $this->db->join('testing_nilai tn','tn.TA_TN='.$this->table.'.TA_AS AND tn.SISWA_TN=ms.ID_SISWA AND tn.JENIS_TN="'.($quran ? 'QURAN' : 'KITAB').'"', 'LEFT');
        $this->db->where(array(
            'AKTIF_AS' => 1,
            'KONVERSI_AS' => 0,
            'NAIK_AS' => NULL,
        ...
        //'L','T','TT','TQ','TK','TTK','TTQ','TQTK','TTQTK','TQTTK','TTQTTK'
        
        if ($quran) {
            $this->db->where('TINGKAT_AS', 16);
            
            $this->db->group_start();
            $this->db->where('LULUS_AS', 'TQ');
    ...
    public function update_nilai($where, $data) {
        $this->db->update('testing_nilai', $data, $where);
