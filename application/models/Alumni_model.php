...
class Alumni_model extends CI_Model {
    ...
    private function _get_table($ALL = FALSE) {
        ...
        $this->db->join('(SELECT * FROM (SELECT * FROM md_nis ORDER BY TA_NIS DESC) mnk GROUP BY SISWA_NIS) mn', $this->table.'.ID_SISWA=mn.SISWA_NIS');
        $this->db->join('akad_siswa asw', $this->table.'.ID_SISWA=asw.SISWA_AS AND asw.TA_AS=mn.TA_NIS AND asw.KONVERSI_AS=0', 'LEFT');
    ...
    public function get_all_ac($where) {
        $this->db->select("ID_SISWA as id, CONCAT(IF(NIK_SISWA IS NULL, 'TIDAK ADA NIK', NIK_SISWA), ' - ',NAMA_SISWA) as text");
        $this->_get_table_simple();
        $this->db->like('NAMA_SISWA', $where);
        $this->db->or_like('NIK_SISWA', $where);
