...
$pdf->Cell(0, 5, 'REKAPITULASI HAFALAN SISWA PERJENJANG', 0, 0, 'C');
$pdf->Ln();
...
foreach ($nama_kelas as $detail) {
    ...
    $pdf->Cell(50, 5, $detail->DEPT_TINGK, 1, 0, 'L');
...
function get_data($kelas, $data) {
    foreach ($data as $detail) {
        if ($kelas->DEPT_TINGK == $detail->DEPT_TINGK) {
            return $detail->data;
        }
    }
    return "-";
