...
foreach ($data['DETAIL_PELANGGARAN'] as $detail) {
    ...
    $pdf->MultiCell(0, 5, 'Anak tersebut telah melakukan perbuatan melanggar tata tertib dan peraturan Perguruan Islam Mathali\'ul Falah dengan akumulasi poin sebanyak ' . $siswa['JUMLAH_POIN_KSH'] . ' poin.');
    $pdf->Ln(2);
    ...
    $pdf->Cell(0, 5, ': Bertemu dengan wali kelas untuk mengkomunikasikan perkembangan Santri  Bapak');
    $pdf->Ln(7);
...
function cetak($pdf, $siswa, $data, $nomor_surat) {
    ...
    $pdf->Cell($penerima_margin);
    $pdf->Cell(0, 5, ($siswa['PONDOK_SISWA'] == NULL || $siswa['PONDOK_SISWA'] == 1) ? $siswa['ALAMAT_SISWA'] . ', Kec. ' . $siswa['NAMA_KEC'] : $siswa['ALAMAT_MPS']);
    $pdf->Ln();

    $pdf->Cell($penerima_margin);
    $pdf->Cell(0, 5, ($siswa['PONDOK_SISWA'] == NULL || $siswa['PONDOK_SISWA'] == 1) ? str_replace('Kabupaten', 'Kab.', $siswa['NAMA_KAB']) : '');
    $pdf->Ln();
