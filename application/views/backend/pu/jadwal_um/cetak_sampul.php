...
foreach ($data as $detail) {
    ...
    foreach ($denah as $jk => $data_denah) {
        ...
        foreach ($data_denah['DENAH'] as $ruang => $value) {
            ...
            $pdf->Image(base_url($this->pengaturan->getLogo()), 88, 67, 40, 41, '', '');
            ...
            $pdf->Cell(0, 4, 'SOAL UJIAN MASUK '.$jk_text, 0, 0, 'C');
            $pdf->Ln(8);
            $pdf->SetFont('Arial', 'B', $size_font + 5);
            $pdf->Cell(0, 4, 'PSB TAHUN '.$tahun, 0, 0, 'C');
            $pdf->Ln(70);
            ...
            $pdf->Ln(7);
            $pdf->SetFont('Arial', 'B', $size_font + 7);
            $pdf->Cell(0, 4, $data_pengawas->NIP_PEG.' - '.$this->cetak->nama_peg_print($data_pengawas), 0, 0, 'C');
            $pdf->Ln(12);
            }
