    ...
    <div class="hpanel">
        <div class="panel-body">
            ...
            <h2 class="font-light m-b-xs">
                ATURAN JADWAL TESTING KITAB
            </h2>
            <small>Pembuatan aturan jadwal untuk testing baca kitab</small>
        </div>
    </div>
</div>
                ...
                <div class="panel-body">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <?php $this->generate->input_select2('Kitab '.$i, array('name' => 'MAPEL_TM[]','id' => 'MAPEL_TM_'.$i, 'url' => site_url('master_data/matapelajaran/auto_complete')), TRUE, 5, FALSE, $DETAIL_MAPEL->MAPEL_TM == NULL ? NULL : array('id' => $DETAIL_MAPEL->MAPEL_TM, 'text' => $DETAIL_MAPEL->NAMA_MAPEL)); ?>
                        </div>
                    </div>
                    <?php } ?>
                </div>
...
<script type="text/javascript">
    ...
    function action_save_denah(id) {
        ...
        var action = function(isConfirm) {
            if(isConfirm) {
                <?php if(count($WAKTU['L']) == 0 || count($WAKTU['P']) == 0 || count($RUANG['L']) == 0 || count($RUANG['P']) == 0 || count($MAPEL) == 0) { ?>
                    create_form_ajax('<?php echo site_url('pu/kitab/tambah_aturan') ?>', id, success, message);
                <?php } else { ?>
                    create_form_ajax('<?php echo site_url('pu/kitab/simpan_aturan') ?>', id, success, message);
                <?php } ?>
            }
        };
