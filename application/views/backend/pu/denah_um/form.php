                ...
                <div class="panel-body">
                    <div class="row">
                        ...
                        <div class="col-md-2 col-md-offset-3">
                            <button type="button" class="btn btn-danger btn-block" id="btn_reset" onclick="reset_aturan();" disabled="true">RESET ATURAN</button>
                        </div>
                        <div class="col-md-2">
                            <button type="submit" class="btn btn-primary btn-block" id="btn_buat" disabled="true">BUAT DENAH</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    function change_capacity(t) {
        ...
        if((parseInt(kapasitas) == 0) || (kapasitas == "")) $("#btn_request").prop('disabled', true);
        else $("#btn_request").removeAttr('disabled');
    }
    ...
    function proses_aturan() {
        ...
        var success = function(data) {
            if(data.status) {
                $("#btn_buat, #btn_reset").removeAttr('disabled');
    ...
    function build_tag_html(data, key, detail, ruang, key_ruang_sisa, jk) {
        ...
        if(detail === 'Data') {
            if(key === (key_ruang_sisa - 1)) tag_html += '<td>' + (key + 1) + '.&nbsp;&nbsp;&nbsp;SISA</td>';
            else tag_html += '<td>' + (key + 1) + '.&nbsp;&nbsp;&nbsp;Ruang ' + ruang[key]['KODE_RUANG'] + '</td>';
        } else {
            tag_html += '<td>' + detail + '</td>';
        }
        
        $.each(data, function(index, item){
            if((detail === 'Data') || (detail === 'Jumlah')) total += parseInt(item);
            ...
            if(detail === 'Jumlah') {
                ...
                if(detail === 'Data') tag_html += '<input type="text" class="form-control input-sm data_nilai data_' + key + '_' + index + '_' + jk + ' col_' + index + '_' + jk + ' row_' + key + '_' + jk + '" style="width:40px" name="aturan_' + jk + '[' + key + '][' + index + ']" value="' + item + '" data-nilai="' + item + '" onchange="change_value(' + key + ', ' + index + ', \'' + jk + '\')"/>';
                else tag_html += item;
        ...
        if(detail === 'Jenjang') tag_html += '<td>#</td>';
        else if(detail === 'Tingkat') tag_html += '<td>Total</td>';
        else if((detail === 'Data') || (detail === 'Jumlah')) tag_html += '<td class="data_count count_row_' + key + '_' + jk + '" data-count="' + total + '">' + total + '</td>';
    ...
    function proses_change(status, value, jk) {
        ...
        if(status === 'col') {
            ...
            if(jumlah_peserta === jumlah_peserta_perubahan) $("#btn_buat").removeAttr('disabled');
            else $("#btn_buat").prop('disabled', true);
        }
    }
    ...
    function saving_denah(id) {
        ...
        create_form_ajax('<?php echo site_url($MODE == 'UM' ? 'pu/denah_um/simpan_denah' : 'pu/denah_us/proses_aturan'); ?>', id, success, msg);
    }
